...
Sprite::Sprite(std::shared_ptr<Surface> a_Surface, unsigned int a_NumFrames) :
...
Sprite::~Sprite()
{
	//delete m_Surface; //TODO tried making Sprite shared in memory
...
//Call instead of Draw to flipp the Sprite horizontally. 
void Sprite::DrawFlipped(Surface* a_Target, int a_X, int a_Y)
{
	if ((a_X < -m_Width) || (a_X > (a_Target->GetWidth() + m_Width))) return;
	if ((a_Y < -m_Height) || (a_Y > (a_Target->GetHeight() + m_Height))) return;
	...
	if ((x2 > x1) && (y2 > y1))
	{
		...
		for (int y = 0; y < height; y++)
		{
			...
			for (int x = xs; x < width; x++)
			{
				int srcX = m_Width - x - 1; // Added this line
				const Pixel c1 = *(src + srcX); // Modified this line
				if (c1 & 0xffffff) *(dest + addr + x) = c1;
			}
