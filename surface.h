...
public:
	...
	// Structors
	//Sprite(Surface* a_Surface, unsigned int a_NumFrames, bool flipped);
	Sprite(std::shared_ptr<Surface> a_Surface, unsigned int a_NumFrames); //Tried fixing a error i had. From what i can figure out it is because i have multiple sprites assigned to the surface. 
																		  //when the sprite is being deleted it deletes the surface and than on the next sprite it obviously cant find its surface in memory
	...
	void DrawFlipped(Surface* a_Target, int a_X, int a_Y);
...
private:
	...
	std::shared_ptr<Surface> m_Surface; // Updated to use std::shared_ptr<Surface>
	//bool m_Flipped;
};
